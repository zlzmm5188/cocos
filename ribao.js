    ...
    try {
        ...
        if (!userInfo) {
            if (typeof ApiClient !== 'undefined') {
                try {
                    userInfo = await ApiClient.get('/user/user/index');  // ✅ 使用正确的后端路径
                } catch (e) {
        ...
        if (typeof ApiClient !== 'undefined') {
            try {
                headData = await ApiClient.get('/user/ribao/head');  // ✅ 使用正确的后端路径
            } catch (e) {
...
async function loadProfitRecords() {
    ...
    try {
        ...
        if (typeof ApiClient !== 'undefined') {
            try {
                response = await ApiClient.get('/user/ribao/list', {  // ✅ 使用正确的后端路径
...
async function handleTransferIn() {
    ...
    try {
        ...
        let response = null;
        if (typeof ApiClient !== 'undefined') {
            response = await ApiClient.post('/user/ribao/in', {  // ✅ 使用正确的后端路径
...
async function handleTransferOut() {
    ...
    try {
        ...
        let response = null;
        if (typeof ApiClient !== 'undefined') {
            response = await ApiClient.post('/user/ribao/out', {  // ✅ 使用正确的后端路径
                amount: amount,  // ✅ 使用 amount 而不是 money
                currency: 'CNY',  // ✅ 添加币种参数
                pay_password: ''  // 注意：ribao.js中没有密码管理，需要更新前端流程
            });
