...
async function checkCheckinStatus() {
    try {
        ...
        const response = await fetch(`${API_BASE}/user/sign/info`, {
            method: 'GET',
            headers: {
                // 'Token': token,
        ...
        if (text.trim().startsWith('<!DOCTYPE') || text.trim().startsWith('<html') || text.includes('系统发生错误') || text.includes('ThinkPHP')) {
            console.warn('[API] /user/sign/info 返回HTML错误页面');
            return null;
        }
...
async function doCheckin() {
    try {
        ...
        const API_BASE = window.API_CONFIG?.baseURL || 'https://api.4kp3l0iq.top';
        const response = await fetch(`${API_BASE}/user/sign/sign`, {
            method: 'POST',
            headers: {
                // 'Token': token,
                'Content-Type': 'application/json'
        ...
        // 检查是否是HTML错误页面
        if (text.trim().startsWith('<!DOCTYPE') || text.trim().startsWith('<html') || text.includes('系统发生错误') || text.includes('ThinkPHP')) {
            console.warn('[API] /user/sign/sign 返回HTML错误页面');
