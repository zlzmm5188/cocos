...
async function loadInviteInfo() {
    try {
        // 修复API路径：正确的是 /user/user/invite
        const data = await apiRequest('/user/user/invite', {}, 'GET');
...
async function loadAllTeamData() {
    ...
    try {
        // 1. 先加载一级团队（直推）
        const level1Data = await apiRequest('/user/team/team', {
            page: 1,
            uid: 0,
            level: 1
        }, 'POST');
        ...
        if (level1Data.data && level1Data.data.users) {
            ...
            for (const member of userData.teamData.level1) {
                try {
                    const level2Data = await apiRequest('/user/team/team', {
                        page: 1,
                        uid: member.id,
                        level: 2
                    }, 'POST');
...
async function loadMemberSubTeam(memberId) {
    try {
        const data = await apiRequest('/user/team/team', {
            page: 1,
            uid: memberId,
            level: 2
        }, 'POST');
