...
async function createPayment(params) {
    ...
    try {
        ...
        try {
            ...
            if (text1.trim().startsWith('<!DOCTYPE') || text1.trim().startsWith('<html') ||
                text1.includes('系统发生错误') || text1.includes('ThinkPHP')) {
                throw new Error('服务器返回错误页面');
            }
            result = JSON.parse(text1);
        } catch (formError) {
            // 如果表单格式失败，尝试JSON格式
            ...
            if (text2.trim().startsWith('<!DOCTYPE') || text2.trim().startsWith('<html') ||
                text2.includes('系统发生错误') || text2.includes('ThinkPHP')) {
                throw new Error('服务器返回错误页面');
            }
